...
// stubScheduler mirrors the real scheduler's contract that matters for this test:
// ExecuteTask runs the supplied watch function in its own goroutine, which is what
// concurrently reads the task index map.
...
// TestExecuteTaskIndexMapNoRace runs the concurrent executor with many children so
// the per-child watcher goroutines read taskIdxMap while the executor spawns the

...
func (cache *BlockCache) GetCachedBlocks() []*Block {
	...
	for _, slot := range slots {
		for _, block := range cache.numberMap[slot] {
...
func (cache *BlockCache) IsCanonicalBlock(blockRoot common.Hash, headRoot common.Hash) bool {
	res, _ := cache.GetBlockDistance(blockRoot, headRoot)
	return res
}

func (cache *BlockCache) GetBlockDistance(blockRoot common.Hash, headRoot common.Hash) (bool, uint64) {
